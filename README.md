...
Vercel 会将 `api/recognize.js` 部署为无状态函数。页面通过同源 HTTPS 请求将凭据和图片交给该函数，函数仅在内存中用凭据生成 SimpleTex 请求签名，随后立即丢弃；应用不会将凭据写入文件、数据库或日志。公式渲染资源已固定并随项目部署，不会在运行时从第三方 CDN 加载。
